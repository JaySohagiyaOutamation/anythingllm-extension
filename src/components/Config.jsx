...
export default function Config({ status, onStatusChange }) {
  ...
  async function disconnectFromExtension() {
    ...
    onStatusChange();
    setSaveStatus("Successfully disconnected from Outamation AI");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "Outamation AI is currently offline. Please try again later."
        );
        return;
      }
      ...
      onStatusChange();
      setSaveStatus("Successfully connected to Outamation AI");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
        ...
        <div className="w-full flex flex-col gap-y-4">
          <div className="flex flex-col w-full">
            <label className="text-black text-sm font-semibold block mb-3">
              Outamation AI Connection String
            </label>
            <input
              type="text"
              ...
              className="bg-black/70 text-white placeholder:text-white/40 text-sm rounded-lg focus:outline-[#46C8FF] active:outline-[#46C8FF] outline-none block w-full p-2.5"
            />
          </div>
          <button
