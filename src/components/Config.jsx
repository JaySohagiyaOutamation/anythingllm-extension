...
export default function Config({ status, onStatusChange }) {
  ...
  async function disconnectFromExtension() {
    ...
    onStatusChange();
    setSaveStatus("Successfully disconnected from OutamationLLM");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "OutamationLLM is currently offline. Please try again later."
        );
        return;
      }
      ...
      onStatusChange();
      setSaveStatus("Successfully connected to OutamationLLM");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
        ...
        <div className="w-full flex flex-col gap-y-4">
          <div className="flex flex-col w-full">
            <label className="text-black text-sm font-semibold block mb-3">
              OutamationLLM Connection String
            </label>
            <input
              type="text"
