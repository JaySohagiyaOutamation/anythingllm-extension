...
export default function Config({ status, onStatusChange }) {
  ...
  async function disconnectFromExtension() {
    ...
    onStatusChange();
    setSaveStatus("Successfully disconnected from OutamationAI");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "OutamationAI is currently offline. Please try again later."
        );
        return;
      }
      ...
      onStatusChange();
      setSaveStatus("Successfully connected to OutamationAI");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
        ...
        <div className="w-full flex flex-col gap-y-4">
          <div className="flex flex-col w-full">
            <label className="text-black text-sm font-semibold block mb-3">
              OutamationAI Connection String
            </label>
            <input
              type="text"
